...
#   timestamp: 2025-12-30T14:05:11+00:00
...
class GpuDevice(BaseModel):
    name: Optional[str] = Field(
        None, description='GPU model name', examples=['NVIDIA A100']
    )
    memory_mb: Optional[int] = Field(
        None, description='Total GPU memory in MB', examples=[40960]
    )


class Status(Enum):
    """
    High-level GPU readiness status
    """
...
class GpuStatus(BaseModel):
    platform: str = Field(
        ..., description='Host platform (e.g., Linux, Darwin)', examples=['Linux']
    )
    gpu_supported_platform: bool = Field(
        ...,
        description='Whether GPU acceleration is supported on this platform (Linux only)',
        examples=[True],
    )
    has_nvidia_gpu: bool = Field(
        ..., description='Whether an NVIDIA GPU was detected', examples=[True]
    )
    ...
    driver_version: Optional[str] = Field(
        None, description='Detected NVIDIA driver version', examples=['550.54.14']
    )
    detected_gpus: Optional[List[GpuDevice]] = Field(
        None, description='List of detected GPU devices'
    )
    gpu4pyscf_installed: bool = Field(
        ...,
    ...
    installed_package: Optional[str] = Field(
        None,
        description='Name of the installed gpu4pyscf package variant (if any)',
        examples=['gpu4pyscf-cu12'],
    )
    recommended_package: Optional[str] = Field(
        None,
        description='Recommended gpu4pyscf package for the detected CUDA version',
        examples=['gpu4pyscf-cu12'],
    )
    status: Status = Field(
        ..., description='High-level GPU readiness status', examples=['ready']
    )
    message: Optional[str] = Field(
        None, description='Human-readable status or error message'
    )
    last_checked: Optional[AwareDatetime] = Field(
        None, description='ISO8601 timestamp of last GPU status refresh'
    )
    logs: Optional[List[str]] = Field(
        None, description='Diagnostic logs from detection or installation'
    )
...
class GpuInstallRequest(BaseModel):
    ...
    enable_gpu: Optional[bool] = Field(
        True, description='Enable GPU acceleration after successful installation'
    )


class GpuInstallJobStatus(Enum):
    """
    Status of a gpu4pyscf installation job
    """
...
class GpuInstallJob(BaseModel):
    job_id: str = Field(..., description='Unique job identifier')
    package: str = Field(..., description='gpu4pyscf package name to install')
    enable_gpu: bool = Field(
        ...,
        description='Whether GPU acceleration should be enabled after successful install',
    )
    status: GpuInstallJobStatus
    error: Optional[str] = Field(
        None, description='Error message if the job failed or was canceled'
    )
    logs: Optional[List[str]] = Field(None, description='Logs from the install attempt')
    created_at: AwareDatetime = Field(..., description='Job creation time')
    started_at: Optional[AwareDatetime] = Field(
        None, description='When the install started'
    )
    completed_at: Optional[AwareDatetime] = Field(
        None, description='When the install finished'
    )
    result_status: Optional[GpuStatus] = None
...
class GpuInstallJobResponse(BaseModel):
    success: bool = Field(..., examples=[True])
    data: Data12


class Data13(BaseModel):
    jobs: List[GpuInstallJob]


class GpuInstallJobListResponse(BaseModel):
    success: bool = Field(..., examples=[True])
    data: Data13
...
class ChatSession(BaseModel):
    ...
    name: str = Field(
        ...
        examples=['DFT calculation for a water molecule'],
    )
...
class CreateChatSessionRequest(BaseModel):
    name: Optional[str] = Field(
        'New chat', description='Session name/title', max_length=200, min_length=1
    )
